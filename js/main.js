...
function xSlider(elem, con) {
  ...
  targetSelect.forEach((i) => {
    ...
    const allItem = targetSelectCon.querySelectorAll('a,button,input,select');
    let checkOpen;

    if (i && targetSelectCon) {
      ...
      i.addEventListener('keydown', (e) => {
        if (e.code === 'Tab' && !e.shiftKey && checkOpen) {
          e.preventDefault();
          allItem[0].focus();
        } else if (e.code === 'Tab' && e.shiftKey && checkOpen) {
          e.preventDefault();
          allItem[allItem.length - 1].focus();
        }
      });
    }

    allItem[allItem.length - 1].addEventListener('keydown', (e) => {
    ...
    allItem[0].addEventListener('keydown', (e) => {
      if (e.code === 'Tab' && e.shiftKey) {
        e.preventDefault();
        i.focus();
      }
...
function webSearch() {
  ...
  const webSearchAllTarget = webSearch.querySelectorAll('a, button, input, select');
  const searchBtn = document.querySelector('.wrapper .webSearchBtn');
  const searchClose = document.querySelector('.webSearch .searchClose');
  const menuOverlay = document.querySelector('.menuOverlay');
  const mobileSearchBtn = document.createElement('button');
  mobileSearchBtn.classList.add('mobileSearchBtn');
  siteHeader.append(mobileSearchBtn);
  ...
  searchBtn?.addEventListener('keydown', (e) => {
    if (e.code === 'Enter') {
      jsSlideToggle(webSearch);
      checkOpenWebSearch = !checkOpenWebSearch;
      webSearchAllTarget[0].focus();
    }
  });
  ...
  searchClose?.addEventListener('keydown', (e) => {
    if (e.code === 'Tab' && !e.shiftKey) {
      ...
      e.preventDefault();
      webSearchAllTarget[webSearchAllTarget.length - 2].focus();
    } else if (e.code === 'Enter') {
      checkOpenWebSearch = false;
    }
  });

  webSearchAllTarget[0].addEventListener('keydown', (e) => {
    if (e.code === 'Tab' && e.shiftKey) {
      e.preventDefault();
      searchClose.focus();
    }
...
function mainMenuSetup() {
  ...
  const goCenterEl = document.querySelector('.goCenter');
  ...
  headContainer.after(sidebarCtrlBtn);
  ...
  function showSidebar() {
    ...
    setTimeout(() => {
      sidebar.style = `display:block;opacity:1;transform: translateX(0px);`;
  ...
  function hideSidebar() {
    checkOpenMenu = false;
    jsFadeOut(menuOverlay);
    sidebar.style = `display:block;transform: translateX(-100%);`;
    setTimeout(() => {
...
(function () {
  ...
  function scrollCheck() {
    ...
    if (windowWidth >= windowWidthSmall && window.scrollY > headHeight) {
      mainMenu.classList.add('sticky');
      main.style.paddingTop = `${mainMenu.offsetHeight}px`;
    } else {
        ...
        itemAllTarget.forEach((n, i) => {
          if (i > 0) {
            n.addEventListener('keydown', (e) => {
              if (mode === 'A') {
                if ((e.code === 'Tab' && !e.shiftKey && e.target === lastItem && modeABtn[index] !== lastTab) || (e.code === 'Tab' && e.shiftKey && e.target === firstItem)) {
...
function fontSize() {
  ...
  const body = document.querySelector('body');
  if (elB) {
    xSlider('.fontSize.typeB > button', 'ul');
  }
  if (el) {
    list.forEach((v) => {
...
function gotoCenter() {
  ...
  if (goCenterTag) {
    goCenterTag.addEventListener('click', (e) => {
      acTag.focus();
      window.scrollTo({
        top: distance,
        left: 0,
...
document.addEventListener('keydown', (e) => {
  console.log(checkOpenWebSearch);
});
